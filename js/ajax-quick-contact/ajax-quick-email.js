		...
		$.post( ajax_url, data, function(response) {
		  ...
		  if(response) {
			...
			if(response === 'full_name_error') {
				...
				$('#pm_form_response').html(wordpressOptionsObject.successMessage).css({ 'marginTop' : -60 });
				...
				$('#pm_form_response').html(wordpressOptionsObject.failedMessage).css({ 'marginTop' : -60 });
				$('.pm_quick_contact_submit').css({
					'opacity' : 0,
					'visibility' : 'hidden'
				});
